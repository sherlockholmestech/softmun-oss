...
function addSpeaker() {
	...
	const currentList = selectedCommitteeData.value?.gslList
		? [...selectedCommitteeData.value.gslList]
		: [];
	currentList.push(selectedSpeakerInput.value.value);
	updateCommittee(selectedCommittee.value!, { gslList: currentList });
	selectedSpeakerInput.value = null;
}

function removeSpeaker(index: number) {
	if (!selectedCommitteeData.value?.gslList) return;
	const currentList = [...selectedCommitteeData.value.gslList];
...
function reorderSpeaker(fromIndex: number, toIndex: number) {
	const list = selectedCommitteeData.value?.gslList;
...
const listLength = computed(
	() => selectedCommitteeData.value?.gslList.length ?? 0,
);
</script>
...
<template>
	<UCard class="panel-card min-w-0">
		...
		>
			<div
				v-for="(speakerId, index) in selectedCommitteeData!.gslList"
				:key="speakerId"
				data-speaker-row
