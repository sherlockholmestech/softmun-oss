...
const state = ref({
	timeLeft: selectedCommitteeData.value?.unmodTotalTime ?? 90,
	totalTime: selectedCommitteeData.value?.unmodTotalTime ?? 90,
	isRunning: false,
	progress: 100,
...
watch(
	() => selectedCommitteeData.value?.unmodTotalTime,
	(newTime) => {
	...
	<UCard class="panel-card">
		<template #header>
			...
			<p class="text-sm text-slate-500 dark:text-slate-400">
				The time left in the unmoderated caucus.
			</p>
		</template>
